...
import { tokensHost, storageHost, audioTimeoutTime } from './constants.js';
import { playersMapName } from './network-schema/constants.js';
...
export function getRandomString(rng = Math.random) {
  const r = () => rng().toString(36).substring(7);
  return r() + r() + r() + r();
}
...
export async function loadJson(u, { signal } = {}) {
  const res = await fetch(u, {
    signal,
  });
  return await res.json();
}
export async function loadImageBitmap(u, { signal } = {}) {
  const res = await fetch(u, {
    signal,
  });
...
export async function loadAudioBuffer(audioContext, url, { signal } = {}) {
...
export const fetchArrayBuffer = async (srcUrl, { signal } = {}) => {
...
export const cacheBust = (u) => {
  // for supabase
  // if (/^[a-zA-Z0-9]+:\/\//.test(u)) {
  if (/https:\/\/[a-zA-Z0-9]+\.supabase\.co\//.test(u)) {
    u = new URL(u);
    u.searchParams.set('version', makeId(8));
    return u.href;
  } else {
    return u;
  }
...
export const getTopLevelApp = (app) => {
  // handle player/avatar special cases
  if (app?.player?.playerApp) {
    app = app.player.playerApp;
  } else if (app?.player?.avatarApp) {
    app = app.player.avatarApp;
  }

  // scan upward to top-level app
  while (app?.parent && !app?.parent.isAppManager) {
    app = app.parent;
  }
...
export const makeAppQueryFn = (query) => {
  const getAppName = (app) => app?.name || app?.spec?.start_url || '';
  if (query instanceof RegExp) {
    return (app) => query.test(getAppName(app));
  } else if (typeof query === 'string') {
    return (app) => getAppName(app) === query;
  } else if (typeof query === 'function') {
    return (app) => query(app);
  } else {
    return () => true;
  }
};

export const mergeUserSpecAvatarUrl = (userSpec, avatarUrl) => {
  return {
    ...userSpec,
    playerSpec: {
      ...userSpec.playerSpec,
      name: avatarUrl.match(/([^\/\.]*)(?:\.\.*)?$/),
      avatarUrl: avatarUrl,
    },
  };
};
...
export const getCorsWorkerProxyUrl = (u) =>
  `https://cors-proxy.upstreet.ai/${u}`;
export const getCorsFullProxyUrl = (u) =>
  `https://cors-proxy-2.upstreet.ai/${u}`;

export const uniqueArray = (a, keyFn = (e) => e) => {
  const map = new Map();
  for (const e of a) {
    const key = keyFn(e);
    if (!map.has(key)) {
...
/* export const objectEqualsShallow = (a, b) => {
  if (a && b) {
    for (const k in a) {
      if (a[k] !== b[k]) {
        return false;
...
export const arrayFromGenerator = async (generatorCb) => {
  const result = [];
  for (;;) {
    const { done, value } = await generatorCb.next();
...
export const spliceRandom = (array) => {
  const index = Math.floor(Math.random() * array.length);
  const result = array[index];
  array.splice(index, 1);
  return result;
};
export const chooseRandomArray = (array, n = 1) => {
  array = array.slice();

  const result = [];
  for (let i = 0; i < n && array.length > 0; i++) {
    const item = spliceRandom(array);
    result.push(item);
  }
  return result;
};
export const heading = (s) => `# ${s}`;
export const underline = (s) => s + '\n' + Array(s.length + 1).join('-');
export const escapeRegexString = (s) =>
  s.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, '\\$1');
export const letterAt = (i) => {
  // return letter starting at (i = 0) => 'a', (i = 1) => 'b', etc.
  // clamp to a-z
  i = mod(i, 26);
  return String.fromCharCode(97 + i);
};
export const ensureNewline = (s) => {
  // if (s !== '') {
  if (!/\n$/.test(s)) {
    s += '\n';
  }
...
export function splitWithIndex(inputString, regex) {
  let result = [];
  let matchArray = [...inputString.matchAll(regex)];

  if (matchArray.length === 0) {
    // If no matches, return the whole string
    let str = new String(inputString);
    str.index = 0;
    result.push(str);
    return result;
  }
  ...
  matchArray.forEach((match, i) => {
    ...
    if (startIndex !== endIndex) {
      ...
      let str = new String(inputString.substring(startIndex, endIndex));
      str.index = startIndex;
      result.push(str);
    }
    startIndex = endIndex + match[0].length;
    if (i === matchArray.length - 1 && startIndex < inputString.length) {
      // Add the substring after the last match
      let str = new String(inputString.substring(startIndex));
      str.index = startIndex;
      result.push(str);
    }
  });
...
export const recurseApps = (app, fn) => {
  fn(app);
  for (const child of app.children) {
    if (child.isApp) {
      recurseApps(child, fn);
    }
  }
};

export const resumeAudioContext = (audioContext) => {
  if (audioContext.state === 'suspended') {
    audioContext.resume();
  }
}

export async function decodeAudioFile(url, audioContext) {
...
async function delayWithReturnValue(milliseconds = 0, returnValue) {
...
export async function stt(audioBlob) {
  ...
  const res = await fetch(
    `https://${aiProxyHost}/api/ai/audio/transcriptions`,
    {
      method: 'POST',
      body: fd,
      headers: {
