...
import {Allocator, ScratchStackAllocator, ScratchStack} from '../geometry-util.js';
...
physx.waitForLoad =  () => {
  if (!loadPromise) {
    loadPromise = (async () => {
      ...
      const scratchStackSize = 16 * 1024 * 1024;
...
const physxWorker = (() => {
  ...
  w.raycastPhysics = (physics, p, q, maxDist = 1000) => {
    if (physics) {
  ...
  w.raycastPhysicsArray = (physics, p, q, n, maxDist = 1000) => {
    if (physics) {
  ...
  w.doCut = (
    ...
    const allocator = new ScratchStackAllocator(scratchStack)
  ...
  w.meshoptSimplify = (mesh, targetRatio, targetError) => {
  ...
  w.meshoptSimplifySloppy = (mesh, targetRatio, targetError) => {
