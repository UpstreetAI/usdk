...
import {TextureAtlasLoader} from '../../atlasing.js';
...
class Mob {
  constructor(app, srcUrl, mobManager) {
    this.app = app;
    this.subApp = null;
  ...
  async loadApp(mobJsonUrl) {
    ...
    if (json) {
      ...
      const subApp = this.mobManager.importManager.createApp();
      subApp.name = this.app.name;
      subApp.description = this.app.description;
      this.app.add(subApp);
      subApp.updateMatrixWorld();

      const appContext = this.mobManager.engineAppContextFactory.makeAppContext({
        app: subApp,
      });

      await this.mobManager.importManager.createAppAsync({
        start_url: glbUrl,
        app: subApp,
        appContext,
        start_url: glbUrl,
      ...
      let  {idleAnimation = ['idle'], aggroDistance, walkSpeed = 1} = json;
      if (idleAnimation) {
      ...
      for (const physicsObject of physicsObjects) {
        this.mobManager.physicsTracker.addAppPhysicsObject(this.app, physicsObject);
      }
      ...
      this.cleanupFns.push(() => {
        ...
        for (const physicsObject of physicsObjects) {
          this.mobManager.physicsTracker.removeAppPhysicsObject(this.app, physicsObject);
        }
      });
      ...
      if (idleAnimationClips.length > 0) {
        ...
        globalThis.idleAnimation = idleAnimation;
        globalThis.idleAnimationClips = idleAnimationClips;
        globalThis.idleActions = idleActions;
        globalThis.mixer = mixer;
        globalThis.app = this.app;
        globalThis.subApp = subApp;
      }
      ...
      this.updateFns.push((timestamp, timeDiff) => {
        const localPlayer = this.mobManager.playersManager.getLocalPlayer();
...
class MobAIControllerPrototype {
  constructor(){
    ...
    window.mobsAttack = (id)=>{
      ...
      for(const m of mobs){
        m.target = this.mobManager.playersManager.getLocalPlayer();
...
export class MobInstance {
  constructor(pos, quat, geometryIndex, timeOffset, radius, height, velocity, mobManager) {
    this.actions = [];
  ...
  */
  ...
  getLocalPlayerDistance() {
    const player = this.mobManagerplayersManager.getLocalPlayer();
...
const textureAtlasLoader = new TextureAtlasLoader();
class MobBatchedMesh extends InstancedBatchedMesh {
  constructor({
    ...
    } = textureAtlasLoader.createTextureAtlas(meshes, {
...
export class MobManager extends THREE.Object3D {
  ...
  addMobApp(app, srcUrl) {
    ...
    const mob = new Mob(app, srcUrl, this);
    this.mobs.push(mob);
  }
