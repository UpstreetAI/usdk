...
const _loadParticleTextureByName = async (name, u, numFrames, rowSize) => {
  // await waitForLoad();
...
uniform float uRowSize1;
uniform float uRowSize2;
uniform float uRowSize3;
uniform float uRowSize4;
uniform float uRowSize5;
uniform float uRowSize6;
uniform float uRowSize7;
uniform float uRowSize8;
...
vec2 getUv(float numFrames, float rowSize) {
  float f = vTimeDiff;
...
const _makeMaterial = maxNumTextures => {  
  ...
  material.setTextures = newTextures => {
    ...
    for (let i = 0; i < newTextures.length; i++) {
      ...
      const uRowSizeUniform = material.uniforms['uRowSize' + index];
      uRowSizeUniform.value = newTexture.rowSize;
      uRowSizeUniform.needsUpdate = true;

      // console.log('got', {
      //   numFrames: newTexture.numFrames,
      //   rowSize: newTexture.rowSize,
      // });
...
class ParticleSystem extends THREE.InstancedMesh {
  constructor({
    ...
    const material = _makeMaterial(maxParticles);
  ...
  async loadTexture(particleName, particleUrl, numFrames, rowSize) {
    const texture = await _loadParticleTextureByName(particleName, particleUrl, numFrames, rowSize);
    this.textures.push(texture);
    this.material.setTextures(this.textures);
    // return texture;
  }
  ...
  update(timestamp, timeDiff) {
    ...
    const {camera} = this.particleSystemManager;
...
export class ParticleSystemManager /*extends THREE.Object3D*/ {
  constructor({
    camera,
  }) {
    // super();
