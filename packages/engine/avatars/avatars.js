...
class Avatar extends AvatarBase {
    ...
    if (this.options.visemes) {
      ...
      this.skinnedMeshesVisemeMappings = this.skinnedMeshes.map(skinnedMesh => {
        ...
        if (morphTargetDictionary && morphTargetInfluences) {
          let blinkLeftBinds, blinkRightBinds, aBinds, eBinds, iBinds, oBinds, uBinds, neutralBinds, angryBinds, funBinds, joyBinds, sorrowBinds, surpriseBinds, lookup, lookdown, lookleft, lookright;
          if (isVrmVersion0) {
            ...
            surpriseBinds = _getBlendShapeBindsForName('suprise', skinnedMesh) ||
              _getBlendShapeBindsForName('surprise', skinnedMesh) ||
              _getBlendShapeBindsForName('surprised', skinnedMesh);
            lookup = _getBlendShapeBindsForName('lookup', skinnedMesh);
            lookdown = _getBlendShapeBindsForName('lookdown', skinnedMesh);
            lookleft = _getBlendShapeBindsForName('lookleft', skinnedMesh);
            lookright = _getBlendShapeBindsForName('lookright', skinnedMesh);
          } else {
            ...
            lookup = _getBlendShapeBindsForPresetName('lookup');
            lookdown = _getBlendShapeBindsForPresetName('lookdown');
            lookleft = _getBlendShapeBindsForPresetName('lookleft');
            lookright = _getBlendShapeBindsForPresetName('lookright');
          }
          return [
  ...
  updateAvatarPose(avatarPose) {
    // objects
    for (const k in avatarPose) {
      const modelBoneOutput = avatarPose[k];
      if (modelBoneOutput.isObject3D) { // if it's a bone
        const modelBone = this.foundModelBones[k];
        if (modelBone) { // might be missing e.g. eyes
          if (modelBoneOutput.valid === validEnum.OVERWRITE) {
            modelBone.quaternion.multiplyQuaternions(
              modelBoneOutput.quaternion,
              modelBone.initialQuaternion,
            );
            modelBone.updateMatrix();
          } else if (modelBoneOutput.valid === validEnum.PREMULTIPLY) {
            modelBone.quaternion.premultiply(modelBoneOutput.quaternion);
            modelBone.updateMatrix();
          } else {
            // not a valid bone
          }
    ...
    for (const visemeMapping of this.skinnedMeshesVisemeMappings) {
      if (visemeMapping) {
        ...
        avatarPose.morph_aa.valid && _triggerShapes(morphTargetInfluences,aBinds,avatarPose.morph_aa.value);
        avatarPose.morph_left_blink.valid && _triggerShapes(morphTargetInfluences,blinkLeftBinds,avatarPose.morph_left_blink.value);
        avatarPose.morph_right_blink.valid && _triggerShapes(morphTargetInfluences,blinkRightBinds,avatarPose.morph_right_blink.value);
        avatarPose.morph_lookup.valid && _triggerShapes(morphTargetInfluences,lookupBinds,avatarPose.morph_lookup.value);
        avatarPose.morph_lookdown.valid && _triggerShapes(morphTargetInfluences,lookdownBinds,avatarPose.morph_lookdown.value);
        avatarPose.morph_lookleft.valid && _triggerShapes(morphTargetInfluences,lookleftBinds,avatarPose.morph_lookleft.value);
        avatarPose.morph_lookright.valid && _triggerShapes(morphTargetInfluences,lookrightBinds,avatarPose.morph_lookright.value);
      }
    }
  }

  setLocalXrPose(character, poseArray) {
    const [
  ...
  update(timestamp, timeDiff, session) {
    ...
    const _updateVisemes = () => {
      ...
      const volumeValue = Math.min(this.volume, 1);
      ...
      for (const visemeMapping of this.skinnedMeshesVisemeMappings) {
        if (visemeMapping) {
          ...
          if (volumeValue > 0) {
            ...
            _triggerShapes(morphTargetInfluences,aBinds,volumeValue * (0.25 + this.vowels[0]))
    ...
    const _applyDebug = () => {
      ...
      if (this.debugMesh) {
        if (debug.enabled) {
          ...
          if (this.vrm.scene !== this.debugMesh.parent) {
            this.vrm.scene.add(this.debugMesh);
          }
        }
      ...
      if (!this.audioWorker) {
        ...
        const _volume = e => {
          ...
          if (!this.manuallySetMouth) {
            ...
            this.volume = THREE.MathUtils.lerp(this.volume, e.data, lerpFactor);
          }
        };
