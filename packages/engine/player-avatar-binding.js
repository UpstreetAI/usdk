...
export function makeAvatar(app, environmentManager) {
  if (app.appType === 'vrm') {
    ...
    //   debugger;
    // }
    const avatar = new Avatar(app.vrm, {
      fingers: true,
      hair: true,
      visemes: true,
      debug: false,
...
export function applyCharacterActionsToAvatar(character, rig, timestamp) {
  ...
  const inAirAction = character.actionManager.getActionType('inAir');
  ...
  rig.lastInAirTime = inAirAction ? timestamp : rig.lastInAirTime;

  rig.landWithMoving = rig.lastIsMoving;
  ...
  rig.lastIsMoving = inAirAction?.isMoving;
  ...
  rig.manuallySetMouth  = character.avatarActionFace.manuallySetMouth;
  rig.vowels[1] = character.avatarActionFace.manuallySetMouth ? 0 : rig.vowels[1];
  rig.vowels[2] = character.avatarActionFace.manuallySetMouth ? 0 : rig.vowels[2];
  rig.vowels[3] = character.avatarActionFace.manuallySetMouth ? 0 : rig.vowels[3];
  rig.vowels[4] = character.avatarActionFace.manuallySetMouth ? 0 : rig.vowels[4];
...
export function applyMirrorsToAvatar(character, rig, mirrors) {
  ...
  if (closestMirror) {
    ...
    if (mirrorPosition.distanceTo(character.position) < maxMirrorDistance) {
...
export function applyCharacterToAvatar(character, session, rig, timestamp, mirrors) {
  applyCharacterTransformsToAvatar(character, session, rig); // hmd, gamepads
  applyCharacterMetaTransformsToAvatar(character, session, rig); // velocity

  applyCharacterModesToAvatar(character, session, rig); // top, bottom, hands
  applyCharacterActionsToAvatar(character, rig, timestamp); // action states
  applyCharacterChestToAvatar(character, rig); // chest look
  applyCharacterHeadTargetToAvatar(character, rig); // head look
  applyCharacterEyesToAvatar(character, rig) || applyMirrorsToAvatar(character, rig, mirrors); // eye look

  applyFacePoseToAvatar(character, rig); // face pose
  applyCharacterPoseToAvatar(character, rig); // full pose
}
