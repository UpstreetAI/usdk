...
class AppManager extends THREE.Object3D {
  ...
  async loadJson(json) {
    ...
    const promises = objects.map(object => {
      ...
      if (object2.position) {
        object2.position = new THREE.Vector3().fromArray(object2.position);
      }
      if (object2.quaternion) {
        object2.quaternion = new THREE.Quaternion().fromArray(object2.quaternion);
      }
      if (object2.scale) {
        object2.scale = new THREE.Vector3().fromArray(object2.scale);
      }
      return this.addAppAsync(object2);
    });
  ...
  async addAppAsync({
    ...
    if (assetId) {
      console.warn('assetId is deprecated');
      debugger;
    }
    if (position && !position.isVector3) {
      position = new THREE.Vector3().fromArray(position);
    }
    if (quaternion && !quaternion.isQuaternion) {
      quaternion = new THREE.Quaternion().fromArray(quaternion);
    }
    ...
    if (!app.parent) {
      ...
      console.warn(`app already had a parent. if you are managing the app yourself then use the import manager directly and don't try to track it in the app manager.`)
      debugger;
    }
