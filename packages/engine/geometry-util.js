...
export class ScratchStackAllocator {
  constructor(scratchStack) {
    this.scratchStack = scratchStack;
    this.offset = 0;
  }
  ...
  alloc(constructor, size) {
    if (size > 0) {
      const {offset} = this;
      const start = this.scratchStack.u8.byteOffset + offset;
      const b = new constructor(
        this.scratchStack.u8.buffer,
        start,
        size
      )
      b.offset = offset

      this.offset += size * constructor.BYTES_PER_ELEMENT;

      return b;
    } else {
      return new constructor(this.scratchStack.u8.buffer, 0, 0)
    }
  }
}
