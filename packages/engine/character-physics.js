...
/** @typedef {import('./character-controller.js').LocalPlayer} LocalPlayer */
/** @typedef {import('./character-controller.js').RemotePlayer} RemotePlayer */
/** @typedef {LocalPlayer | RemotePlayer} Character */
...
export class CharacterPhysics {
  ...
  loadCharacterController(characterWidth, characterHeight) {
    ...
    this.capsuleRadius = characterWidth / 2;
    // if width > height, we get an sphere, which is not very accurate.
    this.capsuleHeight = Math.max(characterHeight - characterWidth, 0);

    let contactOffset = 0.01 * this.capsuleHeight;
    let stepOffset = 0.1 * this.capsuleHeight;
  ...
  applyCharacterPhysicsDetail(velocityAvatarDirection, updateRig, now, isLocalPlayer, isNoclip, timeDiffS) {
    if (this.character.avatar) {
      ...
      const _updateAir = () => {
        ...
        const _updateInAir = () => {
          if (this.inAir) {
            // if we are in the air but not jumping, add the inAir action
            if (!localPlayer.actionManager.hasActionType('inAir')) {
              const newInAirAction = {
                type: 'inAir',
                time: now,
                isMoving: localPlayer.avatar.idleWalkFactor > 0,
              }
              localPlayer.actionManager.addAction(newInAirAction);
            }
          } else if (localPlayer.actionManager.hasActionType('inAir')) {
            // we are not in the air, remove the inAir action
            localPlayer.actionManager.removeActionType('inAir');
        ...
        const _updateFallLoop = () => {
          if (
            ...
            localPlayer.characterPhysics.inAir &&
            (now - localPlayer.characterPhysics.lastInAirStartTime) > 200
          ) {
            if (fallLoopFromJump) {
      ...
      if (isLocalPlayer && isNoclip) {
        ...
        this.characterController.position.copy(localVector5);

        flags = 0;
      } else {
        flags = physicsScene.moveCharacterController(
          this.characterController,
          localVector3,
          minDist,
          timeDiffS,
      ...
      if (!this.character.actionManager.hasActionType('sit')) {
        ...
        const controlledApp = this.character.appManager.getAppByInstanceId(objInstanceId);
  ...
  applyCharacterPhysics(now, isLocalPlayer, isNoclip, timeDiffS) {
    ...
    if (
      ...
      this.applyCharacterPhysicsDetail(false, true, now, isLocalPlayer, isNoclip, timeDiffS);
    } else {
      this.applyCharacterPhysicsDetail(true, true, now, isLocalPlayer, isNoclip, timeDiffS);
    }
    // }
  }
  ...
  update(now, timeDiffS, isLocalPlayer, isNoclip, xrAvatarPose) {
    const nowS = now / 1000;
    this.updateVelocity(timeDiffS);
    this.applyGravity(nowS, timeDiffS);
    this.applyCharacterPhysics(now, isLocalPlayer, isNoclip, timeDiffS);
  ...
  destroy() {
    if (this.characterController) {
      this.character.physicsTracker.removeAppPhysicsObject(this.character.parent, this.characterController);
      const physicsScene = physicsManager.getScene();
      physicsScene.destroyCharacterController(this.characterController);
      this.characterController = null;
    }
