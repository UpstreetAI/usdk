...
import { resumeAudioContext } from './util.js';
// import {
//   defaultPlayerName,
//   defaultPlayerBio,
...
// if the character falls below this, respawn them at 0
const respawnHeight = -100;
  ...
  wear(app, {
    ...
    if (loadoutIndex >= 0 && loadoutIndex < numLoadoutSlots) {
      ...
      const _transplantAppFromWorldToPlayer = () => {
        ...
        // if (!worldAppManager?.transplantApp) {
        //   debugger;
        // }

        // console.log('transplant app', app, this.appManager);

        let worldAppManager = app;
        while (worldAppManager && !worldAppManager.isAppManager) {
          worldAppManager = worldAppManager.parent;
        }
        if (!worldAppManager) {
          console.warn('no worldAppManager', {
            app,
            worldAppManager,
          });
          debugger;
        ...
        if (worldAppManager) {
          ...
          debugger;
          console.warn('transplant of application not inside an app manager', {
            app,
            worldAppManager,
          });
        }
  ...
  unwear(app, {
    ...
    if (wearAction) {
      ...
      const _transplantAppFromPlayerToWorld = () => {
        ...
        const worldAppManager = this.engine.appManagerContext.getAppManagers()
          .find(appManager => appManager.isEditable());
...
class AvatarCharacter extends StateCharacter {
  constructor(opts) {
    ...
    this.avatarActionFace = new AvatarCharacterFace(this);
  ...
  async setPlayerSpec(playerSpec) {
    if (playerSpec) {
      ...
      const _loadVoicePack = async () => {
        let voicePackName = playerSpec.voicePack;
        if (!voicePackName) {
          const defaultVoicePackName = this.voices.voicePacks[0].name;
          voicePackName = defaultVoicePackName;
          if (!voicePackName) {
            throw new Error('no voice pack name to default to');
          }
        }
  ...
  getAvatarPose() {
    return this.avatarPose;
  }
  setAvatarPose(avatarPose) {
    this.avatarPose = avatarPose;
  }

  updateAvatar(timestamp, timeDiff, isLocalPlayer, isNoclip, frustum, session, xrAvatarPose) {
    if (this.avatar) {
      const timeDiffS = timeDiff / 1000;

      this.characterBehavior.update(timestamp, timeDiff);
      this.characterPhysics.update(timestamp, timeDiffS, isLocalPlayer, isNoclip, xrAvatarPose);

      if (session && xrAvatarPose) {
        this.avatar.setLocalXrPose(this, xrAvatarPose);
      }
      this.characterHitter.update(timestamp, timeDiffS);
      this.avatarCharacterSfx.update(timestamp, timeDiffS);

      /* const _frustumIntersection = () => {
        // set the bounding box
        localBox.setFromCenterAndSize(
          localVector.copy(this.position)
            .add(localVector2.set(0, -this.avatar.height / 2, 0)),
          localVector3.set(
            this.characterPhysics.capsuleRadius,
            this.characterPhysics.capsuleHeight,
            this.characterPhysics.capsuleRadius,
          ),
        );
      ...
      // if (isLocalPlayer || _frustumIntersection()) {
      if (true) { // XXX broken npc interaction
        if(this.avatar?.object?.scene){
          this.avatar.object.scene.visible = true;
        } else {
          // console.warn('no avatar object scene, this is a known issue on character360 that needs to be resolved')
        }

        // this.actionManager.update(timestamp);
        this.avatarCharacterFx.update(timestamp, timeDiffS);
        this.avatarActionFace.update(timestamp, timeDiffS);

        const mirrors = this.environmentManager.getMirrors();
        applyCharacterToAvatar(this, session, this.avatar, timestamp, mirrors);

        this.avatar.update(timestamp, timeDiff, session);

        if (this.avatarPose) {
          this.avatar.updateAvatarPose(this.avatarPose);
        }
        ...
        this.avatar.object.scene.visible = false;
      }
    }
  }
...
class LocalPlayer extends UninterpolatedPlayer {
  ...
  setMicMediaStream(mediaStream) {
    ...
    if (mediaStream) {
      ...
      mediaStreamSource.connect(this.avatar.getMicrophoneInput());

      this.microphoneMediaStream = mediaStreamSource;
    }
  }

  async playStream(stream, opts) {
    const {audioContext} = this.audioManager;
...
class RemotePlayer extends InterpolatedPlayer {
  ...
  updateAvatar(timestamp, timeDiff) {
    if (this.avatar) {
      ...
      const mirrors = this.environmentManager.getMirrors();
      applyCharacterToAvatar(this, null, this.avatar, timestamp, mirrors);
