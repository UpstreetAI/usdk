...
export class GameManager extends EventTarget {
  ...
  constructor({
    ...
    // members
    if (!engineRenderer || !ioManager || !cameraManager || !playersManager || !loadoutManager || !raycastManager || !storyManager || !interactionManager || !appManagerContext || !hitManager || !dropManager || !sounds || !zTargetingManager) {
  ...
  isInAir() {
    const localPlayer = this.playersManager.getLocalPlayer();
    return localPlayer.characterPhysics.lastInAir;
  }
  ...
  setMouseSelectedObject(o, physicsId, position) {
    ...
    const appManager = appManagers.find(appManager => appManager.isEditable());
  ...
  getSpeed() {
    ...
    const isRunning = (isMobile || this.ioManager.keys.shift) && !isCrouched;
  ...
  update(timestamp, timeDiff) {
    ...
    const _updateThink = () => {
      if (this.ioManager.keys.keyG) {
        const now = performance.now();
        const timeDiff = now - this.ioManager.lastKeysDownTime.keyG;
        if (timeDiff > 1000 && !this.storyManager.getConversation()) {
          this.ioManager.keys.keyG = false;

          (async () => {
            await this.storyManager.inspectSelf();
          })();
        }
      }
    };
