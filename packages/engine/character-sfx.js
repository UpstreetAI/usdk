...
export class AvatarCharacterSfx {
  ...
  update(timestamp, timeDiffS) {
    ...
    const _handleJump = () => {
      ...
      if(!this.character.avatar.inAirState && this.lastInAirState){
        this.playSoundName('land', {
          volume: 0.5,
        });
      }
      this.lastInAirState = this.character.avatar.inAirState;
  ...
  playGrunt(type) {
    if (this.character.voicePack) { // ensure voice pack loaded
      const audioBufferSourceNode = this.character.voicePack.playAction(type);
  ...
  playEmote(type/*, index*/) {
    if (this.character.voicePack) { // ensure voice pack loaded
      const audioBufferSourceNode = this.character.voicePack.playEmote(type/*, index*/);
