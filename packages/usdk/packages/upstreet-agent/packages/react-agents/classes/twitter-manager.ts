...
import { TwitterScraperClient } from './twitter-scraper-client';
...
//     if (!auth) {
//       throw new Error('Twitter bot requires an auth object');
//     }
...
export class TwitterManager extends EventTarget {
  ...
  async addTwitterBot(args: TwitterArgs) {
    const { auth, agent, kv, codecs, jwt } = args;
    let twitterBot;

    if (auth.type === 'api') {
      twitterBot = new TwitterApiClient(auth as TwitterApiAuth, agent, kv, codecs, jwt);
    } else if (auth.type === 'scraper') {
      twitterBot = new TwitterScraperClient(auth as TwitterScraperAuth, agent, kv, codecs, jwt);
    } else {
      throw new Error('Unsupported Twitter authentication type');
    }
  ...
  removeTwitterBot(twitterBot: TwitterBase) {
    twitterBot.destroy();
  }
