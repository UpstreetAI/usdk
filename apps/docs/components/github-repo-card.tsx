...
const GitHubRepoCard = async ({repo}: {repo:string}) => {
    const response = await fetch(`https://api.github.com/repos/${repo}`)
    const repoInfo = await response.json();
    return (
        <nav className="text-sm h-fit w-full p-2">
        <ul className="p-2 flex space-x-6 items-center hover:bg-fd-background transition-all rounded-md">
          <li className="cursor-pointer block">
            <Link
              href="https://github.com/UpstreetAI/upstreet-core"
              target="_blank"
              rel="noreferrer"
            >
                  ...
                  <div className="text-xs flex items-center space-x-2 text-gray-600 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300">
                    <span className="flex items-center space-x-1">
                      <svg
                        ...
                        style={{
                          maskImage:
                            'url("https://mintlify.b-cdn.net/v6.6.0/regular/star.svg")',
                    ...
                    <span className="flex items-center space-x-1">
                      <svg
                        ...
                        style={{
                          maskImage:
                            'url("https://mintlify.b-cdn.net/v6.6.0/regular/code-fork.svg")',
                          maskRepeat: 'no-repeat',
                          maskPosition: 'center center'
                        }}
                      />
