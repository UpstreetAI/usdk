...
async function verifyChannelAccess(channelId: string, token: string): Promise<void> {
...
export async function fetchDiscordThreadsWithMessages(
  ...
  try {
    ...
    if (!threadsResponse.ok) {
      ...
      throw new DiscordAPIError(threadsResponse.status, error.code, error.message);
    }

    const { threads }: { threads: DiscordThread[] } = await threadsResponse.json();

    // Process all threads concurrently
    const threadResults = await Promise.all(
      threads.map(async (thread) => {
        if (thread.parent_id !== channelId) {
          console.log('Skipped thread', thread.id, 'because it does not belong to the channel.');
        ...
        try {
          ...
          console.warn(`Error fetching messages for thread ${thread.id}:`, error);
          return null;
        }
      })
    );
    ...
    return threadResults.filter((result): result is DiscordThreadWithMessages => result !== null);
  } catch (error) {
