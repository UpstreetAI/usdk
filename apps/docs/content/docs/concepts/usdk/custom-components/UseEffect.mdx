---
title: Custom-Components Using UseEffect
...
## Creating custom component using `UseEffect`
...
```tsx
 useEffect(() => {
    agent.say("Hello! I'm here to assist you.");
    setMessage("The agent has sent a greeting.");
  }, [agent]);
